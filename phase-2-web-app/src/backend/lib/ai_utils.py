...
def detect_intent(message: str) -> Tuple[str, Dict[str, str]]:
    ...
    create_patterns = [
        ...
        # Flexible patterns for "addtask", "i add...", etc.
        r'(?:i )?(?:want to |need to )?add ?(?:task )?(.+)',
        r'(?:i )?(?:want to |need to )?create ?(?:task |todo )?(.+)',
        r'(?:i )?(?:want to |need to )?remind me to (.+)',
        r'(?:i )?(?:want to |need to )?do (.+)',
    ]
